 ...
 */
class Pattern(val pid:Int,val edges:Seq[(VertexId,VertexId)],val vertices:Seq[VertexId])extends Serializable{
...
//hiveContext.sql("select count(*) from tpin_pattern_month_wwd ").rdd.map(row=> row.getAs[Long](0)).collect
  ...
  def apply(pid:Int,edges:Seq[(VertexId,VertexId)],vertices:Seq[VertexId])={
  ...
  // 保存关联交易模式（单向环和双向环）到Hive
  def saveAsHiveTable(hiveContext: HiveContext, pattern: RDD[Pattern]): Unit = {
    // 构建边集字段（边间分号分隔，边内起点终点逗号分隔）
    def constructEdgesString(edges:Seq[(VertexId,VertexId)]): String = {
      var toReturn = ""
      for ((src,dst)<-edges) {
    ...
    hiveContext.createDataFrame(pattern.zipWithIndex.
      ...
      .write.insertInto("tpin_pattern_wwd")
  }
