...
import com.tpin.entity.{VertexType, EdgeAttr, VertexAttr}
...
object InputOutputTools {
  ...
  def getFromCsv(sc: SparkContext): Graph[VertexAttr, EdgeAttr] = {
  ...
  def saveAsHiveTable(hiveContext: HiveContext, tpinWithCommunityId: Graph[VertexAttr, EdgeAttr]): Unit = {
    // 顶点表名称
    val vertexTableName = "tpin_vertex_w_IL"
    // 边表名称
    val edgeTableName = "tpin_edge_w_IL"
    //val hiveContext = new HiveContext(sc)
    //    hiveContext.sql("truncate table tpin_vertex_w_IL")
    //    hiveContext.sql("truncate table tpin_vertex_w_IL")
    //      hiveContext.sql("SELECT count(*) FROM tpin_vertex_w_IL").rdd.map(_.getAs[Long](0)).collect()
    hiveContext.sql("truncate table "+vertexTableName)
    hiveContext.sql("truncate table "+edgeTableName)
    // 顶点表结构
    ...
    // 顶点集写入顶点表
    hiveContext.createDataFrame(tpinWithCommunityId.vertices.map{case (vid,vattr) => Row(
      vid,vattr.vertex_type.toInt,vattr.nsrsbh,vattr.ishuman,vattr.community_id)}, vertexStructType)
      .write.insertInto(vertexTableName)
    // 边集写入边表
    hiveContext.createDataFrame(tpinWithCommunityId.edges.map{case Edge(src,dst,eattr) => Row(
      src,dst, eattr.w_control,eattr.w_tz,eattr.w_IL,eattr.w_trade,eattr.community_id,eattr.trade_je,eattr.tz_je,eattr.se)}, edgeStructType)
      .write.insertInto(edgeTableName)
  }
  ...
  def getFromHiveData(hiveContext: HiveContext): Graph[VertexAttr, EdgeAttr] = {
    ...
    val investmentEdges = investmentRelations_man.union(investmentRelations_company)
      ...
      .map{case  (dstsbh, ((srcindex,w_tz,tzje),dstindex)) =>
        ...
        edgeAttr.tz_je=tzje
        ((srcindex,dstindex),edgeAttr)}.persist
  ...
  def getFromHiveData_withMC(hiveContext: HiveContext): Graph[VertexAttr, EdgeAttr] = {
    //annotation of david:纳税人扩展表提取法定代表人、纳税人
    // 两个ID均不可能为空，且因为经常有个人控制个人，使得ID不能直接作为VertexID，仍然需要重新编号
    val controlRows = hiveContext.sql("SELECT fddbrzjhm, nsrsbh FROM wd_nsr_kz")
      .rdd.filter(row => row.getAs("fddbrzjhm") != null &&
    ...
    //annotation of david:纳税人投资方表提取投资方、纳税人和投资比例（条件：编号合法、投资比例大于0小于等于1）
    val investmentRows_man = hiveContext.sql("SELECT zjhm, nsrsbh,tzbl,tzje FROM nsr_tzf,wd_nsr_kz " +
      "WHERE nsr_tzf.nsrdzdah = wd_nsr_kz.nsrdzdah and TZFXZ_ID LIKE '4%'")
      ...
      row.getAs("tzbl") != null && row.getAs[String]("tzbl").toDouble > 0.0 &&
      row.getAs[String]("tzbl").toDouble <= 1.0).persist
    // 投资方
    ...
    // 投资关系（权重大于0小于等于1）
    val investmentRelations_man = investmentRows_man.map(row =>
      ((row.getAs[String]("zjhm"), row.getAs[String]("nsrsbh")), (row.getAs[String]("tzbl").toDouble,row.getAs[String]("tzje").toDouble)))
      .reduceByKey{case (a, b) => (a._1+b._1,a._2+b._2)}
      .map(relation => if (relation._2._1 < 1.0) relation else ((relation._1._1, relation._1._2), (1.0,relation._2._2))).persist


    // 纳税人投资方表提取投资方、纳税人和投资比例（条件：编号合法、投资比例大于0小于等于1）
    val investmentRows_company = hiveContext.sql("SELECT zjhm, nsrsbh,tzbl,tzje,TZFXZ_ID FROM nsr_tzf, wd_nsr_kz WHERE nsr_tzf.nsrdzdah = wd_nsr_kz.nsrdzdah")
      .rdd.filter(row =>
      !row.getAs[String]("TZFXZ_ID").startsWith("4") &&
        row.getAs("zjhm") != null &&
    ...
    // 发票表提权销方纳税人、购方纳税人和交易金额（条件：编号合法、交易金额大于0）
    val tradeData = hiveContext.sql("SELECT xfnsrsbh, gfnsrsbh,se,je FROM ls_dxj_2015_fp_fwsk_cglmx").rdd.filter(row =>
      row.getAs("xfnsrsbh") != null && row.getAs[String]("xfnsrsbh").matches("[0-9A-Za-z\\-]{8,}")
        ...
        && row.getAs("se") != null && row.getAs[String]("se").toDouble > 0.0).persist
    // 同一销方纳税人的所有交易金额求和
    val tradeSums = tradeData.map(row => (row.getAs[String]("xfnsrsbh"), row.getAs[String]("je").toDouble)).reduceByKey((amount1, amount2) => amount1 + amount2).persist
    // 交易关系（权重大于0小于等于1）
    val tradeRelations = tradeData.map(row => (row.getAs[String]("xfnsrsbh"),
      (row.getAs[String]("gfnsrsbh"), row.getAs[String]("je").toDouble,row.getAs[String]("se").toDouble)))
  ...
  // 从HDFS获取TPIN
  def getFromHDFS_Object(sparkContext: SparkContext,verticesFilePath:String,edgesFilePath:String): Graph[VertexAttr, EdgeAttr] = {
    // 对象方式获取顶点集
    val vertices = sparkContext.objectFile[(VertexId,VertexAttr)](verticesFilePath)
    // 对象方式获取边集
    val edges = sparkContext.objectFile[Edge[EdgeAttr]](edgesFilePath)
