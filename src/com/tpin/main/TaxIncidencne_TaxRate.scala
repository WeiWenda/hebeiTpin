...
//        GenerateTpin.generateNew(sc,hiveContext)

        ComputeTaxIndicators.PrepareTPINandPatternAndIndicator(hiveContext, sc)

//        InterlockExperiment.FindBestWeightFilter(hiveContext, sc, args)

        //annotation of david:实验1，模式 1 与 模式 2 重叠的数量 （去重为前提）
//        InterlockExperiment.Experiment1(sc:SparkContext,hiveContext:HiveContext)

        //annotation of david:实验2 模式1与模式2的查准率
//        InterlockExperiment.Experiment2(sc:SparkContext,hiveContext:HiveContext)
